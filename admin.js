(function() {
  ...
  function handleCreateAS() {
    ...
    if (!serial || !brand) {
      showASNewMsg("브랜드와 시리얼은 필수입니다.", "error");
      return;
    }
    ...
    var params = new URLSearchParams({
      ...
      intake_phone: document.getElementById("as_intake_phone").value.trim(),
    ...
    fetch(API_URL + "?" + params.toString())
      ...
      .then(function(data) {
        if (data.success) {
          ...
          if (data.claim_code) {
            codeBox.innerHTML = '<div class="panel" style="margin-top:12px; background:#FFF8E7; border-color:#F0C040;"><h3 style="color:#8B6914;">발급된 클레임 코드</h3><div style="font-size:20px; letter-spacing:0.1em; font-weight:600;">' + data.claim_code + '</div><div style="font-size:11px; color:#8B6914; margin-top:6px;">이 코드를 의뢰인에게 전달하세요. 의뢰인이 회원가입 후 마이페이지에서 입력하면 이 시계와 수리 이력이 본인 계정으로 연결됩니다.</div></div>';
          } else {
            codeBox.innerHTML = "";
          }
